...
from ding.envs import BaseEnvManager, DingEnvWrapper, SyncSubprocessEnvManager
...
def wrapped_mario_env(version=0, action=7, obs=1):
    return DingEnvWrapper(
        # 设置mario游戏版本与动作空间
        JoypadSpace(
            gym_super_mario_bros.make("SuperMarioBros-1-1-v" + str(version)),
        ...
        cfg={
            ...
            "env_wrapper": [
                ...
                # 以下是你添加的wrapper（必须在FinalEvalRewardEnv之前）
                # lambda env: NoProgressWrapper(env, max_steps_no_progress=200),  # B_noprogress实验
                lambda env: TimePenaltyWrapper(
                    env, penalty=-0.2, tolerance=3
                ),  # B_timepenalty实验: 惩罚停滞
                # lambda env: CoinRewardWrapper(env),  # B_2a_coin实验: 吃金币奖励
                # lambda env: ScoreRewardWrapper(env, score_weight=0.05),  # B_2b_score实验: 游戏得分奖励
                # 默认wrapper：在评估一局游戏结束时返回累计的奖励，方便统计（必须在最外层）
...
def main(cfg, args, seed=0, max_env_step=int(3e6)):
    ...
    collector_env_num, evaluator_env_num = (
        cfg.env.collector_env_num,
        cfg.env.evaluator_env_num,
    )
    # 收集经验的环境，使用并行环境管理器
    ...
    replay_buffer = AdvancedReplayBuffer(
        cfg.policy.other.replay_buffer, tb_logger, exp_name=cfg.exp_name
    )

    # 设置epsilon greedy
    eps_cfg = cfg.policy.other.eps
    epsilon_greedy = get_epsilon_greedy_fn(
...
if __name__ == "__main__":
    ...
    # 断点续训: 指定要恢复的实验目录名(例如: exp/v1_2a_4f_seed0_251218_170758)
    parser.add_argument(
        "--resume",
        "-r",
        type=str,
        default=None,
        help="Resume training from existing experiment directory",
    )
    args = parser.parse_args()
    # 如果指定了 --resume 参数，使用指定的实验目录(断点续训)
    # 否则按照版本/动作/观测/种子生成新的实验目录名
    if args.resume is not None:
        mario_dqn_config.exp_name = args.resume
        print(f"[Resume Training] Using existing experiment directory: {args.resume}")
    else:
        mario_dqn_config.exp_name = (
            "exp/v"
            + str(args.version)
            + "_"
            + str(args.action)
        ...
        print(
            f"[New Training] Creating new experiment directory: {mario_dqn_config.exp_name}"
        )
